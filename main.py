...
def main():
    ...
    if os.path.exists(out_file_name)!=True:
        ...
        for i in range(20):
            ...
            df_nt = run_naive_transfer_cv(seed, 'WHITE', 'MG', dataset_tl_ccsa, groups, feature_type, **parameters_NT)
            ...
            df_tl_unsup = run_unsupervised_transfer_cv(seed, 'WHITE', 'MG', dataset_tl_ccsa, groups, feature_type, **parameters_TL2)
            df_tl_unsup = df_tl_unsup.rename(columns={"TL_Auc": "TL_unsup"})
            print('Unsupervised is done.')
            df_tl_ccsa = run_CCSA_transfer(seed, 'WHITE', 'MG', dataset_tl_ccsa, groups, CCSA_path, feature_type, **parameters_TL3)
